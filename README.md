...
cd backend
npm install
cp .env.example .env   # fill in DATABASE_URL, REDIS_URL, JWT_SECRET, GROQ_API_KEY
...
- **Evaluated three LLM providers during development** — started with Anthropic, moved to Groq for a genuinely free tier, briefly tried Google Gemini for its instruction-following strength, then reverted to Groq after Gemini's free-tier model was deprecated mid-development with no warning, breaking the live app. That experience directly shaped the scoring design below — the app shouldn't be fragile to which model is behind it, so the parts that can be deterministic, are.
